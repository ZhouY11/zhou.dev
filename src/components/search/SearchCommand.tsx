...
export default function SearchCommand({ posts }: SearchCommandProps) {
  ...
  return (
    <>
      <button
        className="flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm text-fg-muted transition-colors hover:border-white/20 hover:text-fg"
        type="button"
      ...
      >
        <div className="border-b border-border p-4">
          <div className="flex items-center justify-between gap-4">
            ...
            <button
              className="rounded-md px-2 py-1 text-sm text-fg-subtle transition-colors hover:bg-bg-muted hover:text-fg"
              type="button"
              aria-label="关闭搜索"
              onClick={closeDialog}
            >
          ...
          {visiblePosts.length > 0 ? (
            <ul>
              {visiblePosts.map((post) => (
                <li key={post.href}>
                  <a
                    className="block rounded-xl px-4 py-3 transition-colors hover:bg-bg-muted"
                    href={post.href}
                    onClick={closeDialog}
                  >
